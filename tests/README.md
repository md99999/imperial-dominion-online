...
`overflow-test.php`, `catapult-test.php` and `migration-test.php` exit non-zero
when a check fails, so they are safe to wire into CI. `smoke-test.php` prints the byte size of each rendered page; a fatal
